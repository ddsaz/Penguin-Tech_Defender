...
class Ashroom(Tower):
    def __init__(self, x: float, y: float, screen_size: tuple, difficulty: str) -> None:
        ...
        self.name = "Algae Shroom"
        self.images = [pygame.transform.scale(pygame.image.load(os.path.join(
                        "assets", "towers", "algae_shroom", "algae_shroom.png")).convert_alpha(), (self.width, self.height)),
                       pygame.transform.scale(pygame.image.load(os.path.join(
                        "assets", "towers", "algae_shroom", "algae_shroom2.png")).convert_alpha(), (self.width, self.height)),
                       pygame.transform.scale(pygame.image.load(os.path.join(
                        "assets", "towers", "algae_shroom", "algae_shroom3.png")).convert_alpha(), (self.width, self.height)),
                       pygame.transform.scale(pygame.image.load(os.path.join(
                        "assets", "towers", "algae_shroom", "algae_shroom2.png")).convert_alpha(), (self.width, self.height))]
        self.icon_image = pygame.transform.scale(pygame.image.load(os.path.join(
            "assets", "towers", "algae_shroom", "icon.png")).convert_alpha(), (80, 80))
    ...
    def main_attack(self, target, enemies=None, current_tick=0, projectiles=None) -> list:
        ...
        if current_tick - self.last_shot_time > self.shoot_interval:
            self.animating = True
            projectiles.append(Ashroom_projectile(self.x + self.width/2, self.y + self.height/2, target,
                               self.screen_size, self.attack_value, self.attack_type, self.resistance_modifier))
